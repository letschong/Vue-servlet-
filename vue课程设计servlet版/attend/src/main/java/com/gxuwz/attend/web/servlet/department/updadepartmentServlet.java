...
@WebServlet(name = "updadepartmentServlet",urlPatterns = {"/api/updadepartment"})
public class updadepartmentServlet extends BaseServlet {
    ApiResult result = new ApiResult();
    com.gxuwz.attend.service.leaveservice leaveservice = new leaveservieImp();
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String depId = request.getParameter("depId");
        String depName = request.getParameter("depName");
        String depaType = request.getParameter("depaType");
        department department = new department(depId,depName,depaType);
        leaveservice.updadepartment(department);
        result.setCode("200");
