...
<el-card class="box-card">
  <el-row :gutter="20">
    ...
    <el-table :data="classlist" style="width: 100%" border scripe>  
      ...
      <el-table-column prop="fullname" label="用户名"  width="180"></el-table-column>
   ...
   <el-button type="danger" icon="el-icon-delete" @click="removeclsId(scope.row.userid)"></el-button>
    </el-tooltip>
...
<script>
   export default{
     data(){
       return{
         ...
         addFormRules:{
            ...
            telephone:[
             {required:true ,message:'请输入电话号码', rigger:'blur'}
           ],
         },
         //控制修改对话框的显示与隐藏
     ...
     async getclasslist(){
      ...
      addclass(){
        this.$refs.addFormRef.validate(async valid =>{
         // console.log(valid)
          if(!valid) return
           const { data:res}=await this.$http.get('adduser',{
       params: this.addForm
        })
           console.log(res);
           if(res.code!=200){
    ...
    async showEditDialog(userid){
      const { data:res}=await this.$http.post('sleedituser',{userid:userid}) 
      console.log(res)
      if(res.code!=200){
      ...
      editclassInfo(){
         ...
         console.log(valid)
          if(!valid) return
          const { data:res}=await this.$http.get('updauser',{
    ...
    const confirmResult = await this.$confirm('此操作将永久删除该用户信息, 是否继续?', '提示', {
        ...
        if(confirmResult!='confirm'){
         ...
         const { data:res}=await this.$http.post('deleteuser',{userid:userid}) 
          console.log(res)
         if(res.code!=200){
